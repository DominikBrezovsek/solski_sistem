<template>
  <div class="add-subject">
    ...
    <div class="button">
      <label class="button-add" @click="updateSubject">
        Shrani spremembe
      </label>
    </div>
  </div>
  ...
  methods: {
    updateSubject() {
      ...
      if (token != null && this.title != '' && this.description != '') {
        ...
        axios.post(path + 'subjects/update', data)
            .then((response) => {
              if (response.data.success == "true") {
                Swal.fire({
                  title: 'Predmet posodobljen',
                  text: 'Predmet je bil uspešno posodobljen.',
                  icon: 'success',
    ...
    getSubject() {
      ...
      if (token != null && subject != null) {
        ...
        axios.post('https://smv.usdd.company/API/public/api/subjects/get', data)
            .then((response) => {
              if (response.data.subject != null) {
                this.title = response.data.subject[0].subject
                this.description = response.data.subject[0].description
                this.subjectId = response.data.subject[0].id
              }
            })
      }
    }
